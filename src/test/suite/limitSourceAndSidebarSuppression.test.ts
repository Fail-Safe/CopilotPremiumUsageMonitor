...
const documentStub: TestDocument = { createElement: (tag: string) => new Elem(tag), getElementById: (id: string) => byId.get(id), querySelector: (sel: string) => root.querySelector(sel), body: { prepend: (_: Elem) => { } } };
...
suite('Limit source states and sidebar suppression', () => {
    ...
    test('webview shows Included limit source inline with Billing when neither custom nor plan', () => {
        ...
        testGlobal.window = { addEventListener: (t: string, h: any) => { if (t === 'message') msgHandler = h; }, removeEventListener: () => { /* noop */ } };
        testGlobal.acquireVsCodeApi = (): TestVSCodeApi => ({ postMessage: (_: any) => { } });
